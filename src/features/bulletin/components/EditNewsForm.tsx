...
export default function EditNewsForm({
    ...
    const clearedImageRef = useRef(false);
    ...
    const handleFormSubmit: SubmitHandler<UpdateNewsFormData> = async (data) => {
        ...
        try {
            ...
            } else if (clearedImageRef.current) {
                imageUrl = undefined;
            }

            const result = await updateNewsAction({ ...data, imageUrl });
            if (!result.success) {
    ...
    return (
        ...
        >
            <div className="flex flex-col gap-3">
                ...
                <NewsImageDropzone
                    initialUrl={news.imageUrl}
                    onFile={(blob) => {
                        pendingBlobRef.current = blob;
                        clearedImageRef.current = blob === null;
                    }}
                />
                <FormTextField
                    name="imageAlt"
                ...
                <Controller
                    ...
                    render={({ field, fieldState }) => (
                        <div className="flex flex-col gap-2">
                            ...
                            <Textarea {...field} placeholder="Full article body..." className="min-h-40" />
                            {fieldState.error && <p className="text-xs text-destructive">{fieldState.error.message}</p>}
                        </div>
                    )}
                />
                        ...
                        render={({ field, fieldState }) => {
                            ...
                            return (
                                <div className="flex flex-col gap-2">
                                    ...
                                    <InputGroup>
                                        <InputGroupInput
                                            value={field.value ? format(parseISO(field.value), 'MMMM dd, yyyy') : ''}
                    ...
                    <Controller
                        ...
                        render={({ field, fieldState }) => (
                            <div className="flex flex-col gap-2">
                                ...
                                <Combobox
                                    items={contentTypes}
                                    value={contentTypes.find((ct) => ct.id === field.value)?.name ?? null}
                                    onValueChange={(name) =>
                                        field.onChange(contentTypes.find((ct) => ct.name === name)?.id ?? null)
                                    }
                                >
