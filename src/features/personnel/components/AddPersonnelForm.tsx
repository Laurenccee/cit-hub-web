...
import { type PersonnelFormData, personnelSchema } from '../schema/personnel';
...
export default function AddPersonnelForm({
  ...
  const handleFormSubmit: SubmitHandler<PersonnelFormData> = async (data) => {
    ...
    try {
      ...
      const result = await addPersonnelAction({ ...data, profilePictureUrl });
      if (!result.success) {
        if (profilePictureUrl) deleteProfilePictureAction(profilePictureUrl);
